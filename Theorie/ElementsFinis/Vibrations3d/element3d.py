...
class H8Meca3d:
    ...
    def weights(self):
        return self._ws
    def setMaterialProperties(self,E,nu,rho):
        self._E   = E
        self._nu  = nu
        self._rho = rho
    ...
    def mass(self):
        ...
        weights = self.weights()
        for i in range(pts.shape[0]):
            N = self.N(pts[i,0],pts[i,1],pts[i,2])
            rho = self.rho()
            J = self.J(pts[i,0],pts[i,1],pts[i,2])
            M+=dot(transpose(N),N)*weights[i]*J*rho
        return M
    def coordNodes(self):
        coord      = zeros((8,3),'d')
    ...
    def computeStressesInElement(self,du):
        B = self.matrixB(0.,0.,0.);#B1=self.matrixB(0.33,0.33)
        E = self.matrixE()
